...
export const addBadgeToUser = async (userId: string, badgeId: number) => handleSupabaseError(
...
// 유저가 특정 뱃지를 삭제할 수 있음
export const deleteBadge = async (userId: string, badgeId: number) => handleSupabaseError(
  async (userId: string, badgeId: number) => supabase
    .from<definitions['profile_badge_mappings']>('profile_badge_mappings')
    .delete()
